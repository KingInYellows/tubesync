'''
    ...
    -- never the real DOWNLOAD_ROOT/downloads. See
    reference_tvshow-nfo-test-hermeticity for why both patches are needed.
'''
import copy
import logging
import tempfile
...
metadata = all_test_metadata['boring']  # title 'no fancy stuff title', upload_date 2017-09-11
...
class BackfillPlexSidecarsTestCase(TestCase):
    ...
    def test_dry_run_changes_nothing(self):
        with temp_download_root():
            ...
            self.assertEqual(str(media.media_file), str(old_path.relative_to(media_file_storage.location)))
    ...
    def test_apply_produces_exact_target_tree_for_channel_and_playlist(self):
        with temp_download_root():
            ...
            channel_media = Media.objects.create(key='chvid1', source=channel, metadata=metadata)
            ...
            playlist_media = Media.objects.create(key='plvid1', source=playlist, metadata=metadata)
            ...
            playlist_tree = ElementTree.fromstring(playlist_nfo.read_text(encoding='utf-8'))
    ...
    def test_nfo_is_written_even_when_no_rename_is_needed(self):
        ...
        with temp_download_root():
            ...
            media.media_file.name = str(new_path.relative_to(media_file_storage.location))
            media.downloaded = True
            media.save()
    ...
    def test_non_bridge_sources_are_untouched_by_all_bridge_sources(self):
        with temp_download_root():
            ...
            normal_media = Media.objects.create(key='regvid', source=normal_source, metadata=metadata)
            ...
            bridge_media = Media.objects.create(key='bvid', source=bridge_source, metadata=metadata)
    ...
    def test_invalid_source_defaults_env_raises_and_changes_nothing(self):
        with temp_download_root():
            ...
            with override_settings():
                with patch.dict('os.environ', {'MEDIANEST_BRIDGE_SOURCE_DEFAULTS': '{not valid json'}):
