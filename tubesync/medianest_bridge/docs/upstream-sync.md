...
Everything the bridge needs lives in `medianest_bridge/` plus seven
...
fine) or introduced a merge conflict against one of those six files
(expected occasionally, and the specific reason this fork's delta is kept
deliberately small -- a six-file, mostly-few-line delta is cheap to
