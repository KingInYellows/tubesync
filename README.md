...
| TUBESYNC_INDEX_ONLY_SKIP_METADATA | Fork addition (not upstream): skip per-item metadata/thumbnail fetches from YouTube for index-only sources (`download_media` disabled). Defaults to True; set to `false` to restore upstream's unconditional per-item fetch behavior. | True |
